...
import { formatContractDate, formatContractPrice, truncateContractHash, computePubkeyHash } from './contract-format';
...
describe('contract formatting helpers', () => {
	...
	describe('computePubkeyHash', () => {
		it('computes SHA256 hash of public key bytes', () => {
			...
			const hash = computePubkeyHash(testPubkey);
		...
		it('produces consistent hash for same input', () => {
			...
			const hash1 = computePubkeyHash(testPubkey);
			const hash2 = computePubkeyHash(testPubkey);
