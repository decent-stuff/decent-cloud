<script lang="ts">
	...
	function handleEdit(pool: AgentPoolWithStats) {
		// This should be handled by the parent component
		console.log("Edit pool:", pool.poolId);
	}

	function handleDelete(pool: AgentPoolWithStats) {
		// This should be handled by the parent component
		console.log("Delete pool:", pool.poolId);
	}

	function handleAddAgent(pool: AgentPoolWithStats) {
		// This should be handled by the parent component
		console.log("Add agent to pool:", pool.poolId);
	}
</script>
			...
			{#each pools as pool (pool.poolId)}
				<tr class="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
					<th scope="row" class="px-6 py-4 font-medium text-white whitespace-nowrap">
						<a href={`/dashboard/provider/agents/${pool.poolId}`} class="text-blue-400 hover:underline">
							{pool.name}
						</a>
					</th>
					<td class="px-6 py-4">
