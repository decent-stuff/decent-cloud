<script lang="ts">
	...
	async function handleDuplicate(offering: Offering, event: Event) {
		...
		duplicatingId = offering.id;
		error = null;

		try {
			const pubkeyHex = hexEncode(currentIdentity.publicKeyBytes);
			const newOfferingId = `copy-${Date.now()}`;
			const path = `/api/v1/providers/${pubkeyHex}/offerings/${offering.id}/duplicate`;
			const signed = await signRequest(currentIdentity.identity, 'POST', path, { newOfferingId });
			await duplicateProviderOffering(currentIdentity.publicKeyBytes, offering.id, signed.body, signed.headers);
			importSuccess = `Offering duplicated successfully`;
			setTimeout(() => {
				importSuccess = null;
			}, 5000);
			await loadOfferings();
		} catch (e) {
			error = e instanceof Error ? e.message : 'Failed to duplicate offering';
