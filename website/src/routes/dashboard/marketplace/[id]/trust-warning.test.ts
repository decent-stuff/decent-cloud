...
function getTrustBannerType(
	...
	// Backend contract: trust_score is null when completed_contracts == 0,
	// so trust_score nullness is the canonical "no track record" signal
	// (total_contracts alone counts cancelled rentals, which is dishonest).
...
describe('trust banner: new provider', () => {
	it('shows new-provider info when trust_score is null (0 completed contracts)', () => {
		const metrics: TrustMetricsSubset = { trust_score: null, total_contracts: 0 };
		expect(getTrustBannerType(metrics, false)).toBe('new-provider');
	});

	it('shows new-provider info when trust_score is undefined (field omitted by API)', () => {
		const metrics: TrustMetricsSubset = { total_contracts: 0 };
		expect(getTrustBannerType(metrics, false)).toBe('new-provider');
	});

	it('shows new-provider info even if trust score is null and dismissed (new-provider overrides everything)', () => {
		const metrics: TrustMetricsSubset = { trust_score: null, total_contracts: 0 };
...
describe('trust banner: low trust warning', () => {
	it('shows low-trust warning when trust_score < 60 and not dismissed', () => {
		const metrics: TrustMetricsSubset = { total_contracts: 5, trust_score: 45 };
	...
	it('shows no warning when trust_score is exactly 60', () => {
		const metrics: TrustMetricsSubset = { total_contracts: 5, trust_score: 60 };
...
describe('trust banner: no warning', () => {
	it('shows no banner when trust_score >= 60 and contracts exist', () => {
		const metrics: TrustMetricsSubset = { total_contracts: 10, trust_score: 85 };
		expect(getTrustBannerType(metrics, false)).toBe('none');
	});
	...
	it('shows no banner when trust_score is 100', () => {
		const metrics: TrustMetricsSubset = { total_contracts: 20, trust_score: 100 };
		expect(getTrustBannerType(metrics, false)).toBe('none');
	});
});
