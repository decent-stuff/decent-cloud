<script lang="ts">
	...
	import {
		getSupportPortalStatus,
		resetSupportPortalPassword,
	...
	async function loadOnboarding() {
		...
		if (data) {
			...
			if (
				supportHours &&
				!supportHoursOptions.slice(0, -1).includes(supportHours)
			) {
				customSupportHours = supportHours;
				supportHours = "custom";
			}
			supportChannels = data.support_channels
				? JSON.parse(data.support_channels)
				: [];
			...
			if (
				refundPolicy &&
				!refundPolicyOptions.slice(0, -1).includes(refundPolicy)
			) {
				customRefundPolicy = refundPolicy;
				refundPolicy = "custom";
			}
			slaGuarantee = data.sla_guarantee || "";
			const usps = data.unique_selling_points
				? JSON.parse(data.unique_selling_points)
				: [];
			usp1 = usps[0] || "";
			usp2 = usps[1] || "";
			usp3 = usps[2] || "";
			commonIssues = data.common_issues
				? JSON.parse(data.common_issues)
				: [];
		}
	}
	...
	async function loadPortal() {
		if (!currentIdentity?.identity) return;
		portalStatus = await getSupportPortalStatus(
	...
	function removeCommonIssue(i: number) {
		commonIssues = commonIssues.filter((_, idx) => idx !== i);
	}
	...
	async function saveOnboarding(e: Event) {
		...
		if (!currentIdentity?.identity || !currentIdentity?.publicKeyBytes)
			return;
		const finalSupportHours =
			supportHours === "custom" ? customSupportHours : supportHours;
		const finalRefundPolicy =
			refundPolicy === "custom" ? customRefundPolicy : refundPolicy;
		if (!accountEmail?.includes("@")) {
			error =
				"Please add an email address in your Account settings first";
			return;
		}
		if (
			!finalSupportHours ||
		...
		try {
			...
			const onboardingData: Partial<ProviderOnboarding> = {
				...
				unique_selling_points:
					usps.length > 0 ? JSON.stringify(usps) : undefined,
				common_issues:
			...
			const syncSigned = await signRequest(
				currentIdentity.identity,
				"POST",
				syncPath,
			);
			const result = await syncProviderHelpcenter(
				pubkeyHex,
				syncSigned.headers,
			);
	...
	async function saveNotificationsQuiet(): Promise<boolean> {
	...
	async function handleTestChannel(channel: "telegram" | "email" | "sms") {
		...
		try {
			...
			await saveNotificationsQuiet();
			testResult = await testNotificationChannel(
				currentIdentity.identity,
				channel,
			);
			// Refresh usage counts after successful send
			if (testResult.sent) {
				usage = await getNotificationUsage(
					currentIdentity.identity,
				).catch(() => null);
			}
		} catch (e) {
	...
	async function handlePortalReset() {
		...
		try {
			newPassword = (
				await resetSupportPortalPassword(currentIdentity.identity)
			).password;
			await loadPortal();
		} catch (e) {
	...
	async function handlePortalCreate() {
		...
		try {
			newPassword = (
				await createSupportPortalAccount(currentIdentity.identity)
			).password;
			await loadPortal();
		} catch (e) {
		...
		>
			{success}{#if articleUrl}
				{" "}<a
					...
					target="_blank"
					rel="noopener"
					class="underline hover:text-green-200">View article →</a
				>{/if}
		</div>{/if}
	{#if testResult}<div
		...
		>
			<strong>{testResult.sent ? "Sent" : "Failed"}:</strong>
			{testResult.message}
		</div>{/if}
