...
async function handleResponse(contract: Contract, accept: boolean) {
		...
		try {
			...
			const payload: ProviderRentalResponseParams = { accept, memo: memo || undefined };
...
async function handleStatusUpdate(contract: Contract, nextStatus: string) {
		...
		if (nextStatus === "provisioned" && !details) {
			error = "Instance details are required to mark a contract as provisioned";
			return;
		}
		error = null;
		actionMessage = null;
		...
		try {
			const payload: ProvisioningStatusUpdateParams = {
				status: nextStatus,
				instance_details: nextStatus === "provisioned" ? details : undefined,
			};
			...
			await updateProvisioningStatus(contract.contract_id, payload, signed.headers);
			actionMessage = `Updated contract status to ${nextStatus}`;
			await loadData();
		} catch (e) {
			error = e instanceof Error ? e.message : "Failed to update provisioning status";
		} finally {
...
<div class="space-y-8">
	<header>
		...
		<p class="text-white/60">Review new rental submissions and keep provisioning progress up to date</p>
	</header>

	{#if error}<div class="bg-red-500/20 border border-red-500/30 rounded-lg p-4 text-red-300">{error}</div>{/if}
	{#if actionMessage}<div class="bg-emerald-500/15 border border-emerald-500/30 rounded-lg p-4 text-emerald-300">{actionMessage}</div>{/if}
	...
	{:else}
		<section class="space-y-4">
			<div class="flex items-center justify-between">
				<h2 class="text-2xl font-semibold text-white">Pending Requests</h2>
				<span class="text-white/60 text-sm">{pendingRequests.length} awaiting action</span>
			</div>

			{#if pendingRequests.length === 0}
				<div class="bg-white/5 border border-white/10 rounded-xl p-6 text-white/70">No pending rental requests right now.</div>
			{:else}
				<div class="space-y-4">
					{#each pendingRequests as contract}
						<PendingRequestCard
							...
							onMemoChange={(value) => (memoInputs = { ...memoInputs, [contract.contract_id]: value })}
							onRespond={(accept) => handleResponse(contract, accept)}
						/>
					{/each}
				</div>
			{/if}
		...
		<section class="space-y-4">
			<div class="flex items-center justify-between mt-10">
				<h2 class="text-2xl font-semibold text-white">Active Contracts</h2>
				<span class="text-white/60 text-sm">{managedContracts.length} in progress</span>
			</div>

			{#if managedContracts.length === 0}
				<div class="bg-white/5 border border-white/10 rounded-xl p-6 text-white/70">No contracts in provisioning stages.</div>
			{:else}
				<div class="space-y-4">
					{#each managedContracts as contract}
						<ManagedContractCard
							...
							onUpdateStatus={(status) => handleStatusUpdate(contract, status)}
						/>
					{/each}
				</div>
			{/if}
