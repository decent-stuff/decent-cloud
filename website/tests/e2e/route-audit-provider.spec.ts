/**
 * Route audit — PROVIDER routes (`/dashboard/provider/*`, `/dashboard/providers/*`).
 *
...
const PROVIDER_DYNAMIC: { title: string; url: () => string }[] = [
...
authTest.describe('route-audit (provider)', () => {
	...
	authTest.afterAll(async () => {
		...
		printFindingsSummary('provider', findings);
	});

	for (const url of PROVIDER_STATIC) {
	...
	for (const r of PROVIDER_DYNAMIC) {
