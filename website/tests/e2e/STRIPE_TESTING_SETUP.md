...
Ensure `STRIPE_WEBHOOK_SECRET=whsec_test_secret` is set via `scripts/dc-secrets set shared/play STRIPE_WEBHOOK_SECRET=whsec_test_secret`

### Card element doesn't appear
- Verify `VITE_STRIPE_PUBLISHABLE_KEY` is set via `scripts/dc-secrets get shared/play VITE_STRIPE_PUBLISHABLE_KEY`
