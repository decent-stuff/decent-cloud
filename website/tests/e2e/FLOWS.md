...
| Reputation search | ✅ | `@marketplace` | `reputation.spec.ts` | `renders the Reputation heading and search box` · `shows the Top Providers leaderboard as the default view (no longer a dead-end)` — UX-006: the landing view is now a browseable leaderboard, not the bare "Search Reputation" idle prompt. |
| Reputation leaderboard | ✅ | `@marketplace` | `reputation-leaderboard.spec.ts` | `renders honesty-gated ranking with metrics and row navigation` — UX-006 "Top Providers" leaderboard on `/dashboard/reputation`. DB-seeds 3 providers (A: high trust + 3 completed, B: 1 completed, C: 0 contracts): A ranks #1 with trust/completion/volume, C is excluded by the honesty gate (`total_contracts > 0`), and clicking row #1 navigates to A's detail page. |
...
| Rent an offering (dialog → contract) | ✅ | `@rental` | `rent-flow.spec.ts` | `rent an offering → contract appears on the rentals list with a Cancel button` — covered in the **full suite** only; excluded from smoke (6s + complex seeding). The fixture pins `auto_accept_rentals=false` so the contract lands at `requested` on purpose (the MANUAL-review path), which is load-bearing for the wallet math. Wallet-debit math is asserted for real: each rent proves `wallet_balances` drops by exactly `payment_amount_e9s` with a matching `rental_debit` ledger row, and each pre-service cancel proves the balance is restored to its pre-rent value with a `rental_refund` row carrying the full principal (provisioning_completed_at_ns IS NULL at `requested` → full prorated refund). |
| Wallet-paid rental auto-accepts (buy-flow bug fix) | ✅ | `@rental` | `rent-wallet-auto-accept.spec.ts` | `wallet-paid rental with auto_accept=true advances past requested` — the complement of rent-flow: pins `auto_accept_rentals=true` and drives the REAL Rent dialog to prove a wallet-paid contract no longer gets stuck at `requested` (the marketplace buy-flow bug — the wallet branch debited but never called `try_auto_accept_contract`). Asserts the contract advances to `accepted`/`active` (self-provisioned activation sets `provisioning_completed_at_ns` synchronously), `payment_status='succeeded'`, exact wallet debit, then cancels → `cancelled`. No full-refund assertion (cancel at `active` is prorated — rent-flow owns that invariant). |
| REAL cloud-resell provisioning (Hetzner VM) | ⚠️ | `@rental` | `rent-provisioning-real.spec.ts` | `rent → provision → SSH details → cancel (real Hetzner VM)` — **⚠️ GATED: skipped by default.** Rents a REAL cx23 VM (offering 1628, db id) and spends real money, so it is gated behind `E2E_REAL_PROVISIONING=1` and is a clean no-op (1 skipped, 0 run) in every default run + CI. When enabled: signed POST /contracts → polls contract → `active` (180s; ~60s in practice) → asserts `provisioning_instance_details` = `{connection_type:'direct_ssh', public_ip, ssh_port:22}` with NO `gateway_*` fields → asserts the rental detail page renders the SSH command for the IP → cancel → `cancelled` + `cloud_resource` → `deleted`. `afterAll` cancels any leftover non-terminal contract so a VM is never left running. Run with `E2E_REAL_PROVISIONING=1 npx playwright test rent-provisioning-real`. |
...
| Wallet page UI (balance / form / ledger / nav) | ✅ | `@account` `@billing` | `wallet-ui.spec.ts` | `wallet page shows balance, top-up form, and empty ledger for a new user` — balance card, Add Funds form (client-side validation + the local-dev Stripe-unconfigured error path), ledger empty + seeded states, dashboard overview card, sidebar link. API-level coverage (auth, validation, Stripe-unconfigured error) in `wallet-api.spec.ts`. |
| Wallet top-up Stripe Checkout redirect + return banners | ✅ | `@billing` | `wallet-stripe-redirect.spec.ts` | `top-up redirects the browser to the Stripe Checkout URL returned by the API` (E2E-4) — closes the gap left by `wallet-ui.spec.ts`'s local-dev error-path test: intercepts the `POST /wallet/topup` response to inject a fake `checkoutUrl` (external-boundary mock sanctioned by the mock policy) and asserts the browser lands on it (`window.location.href` wiring). Plus `?topup=success` and `?topup=cancel` return-banner renders. The external `https://checkout.stripe.com/**` navigation is fulfilled with a stub; no first-party API code is mocked (GET /wallet stays real). |
...
| Become-provider technical-onboarding hub (`/dashboard/provider/start`) | ✅ | `@provider` | `provider-start-cta.spec.ts` · `provider-onboarding.spec.ts` | `start page presents BOTH provider paths plus the shared onboarding steps` (render + href assertions) · `fresh provider steps through the onboarding hub: each link navigates to a live destination` (interactive click-through with a fresh identity). **NOTE:** this route is a STATIC NAVIGATION HUB, not a multi-step form wizard — it renders link cards to the downstream flows (Path A: cloud accounts; Path B: dc-agent docs; Step 2: create offering; Step 3: support profile) and has no Next buttons / form fields / wizard state of its own. The interactive wizard that creates a `provider_profiles` row lives at `/dashboard/provider/support` (covered by `provider-onboarding-submit.spec.ts` above). `provider-onboarding.spec.ts` adds the interactive dimension `provider-start-cta.spec.ts` lacked: it CLICKS each hub link and asserts the destination mounts for a fresh non-provider account. |
...
|------|------|----------|----------------|
| Stripe SDK (`loadStripe`) script | `fixtures/stripe-mock.ts` | External (js.stripe.com) | ⚠️ **currently unused** — the fixture is retained for future Stripe-Elements flows but has no live consumer in the suite as of 2026-08-11 (the previously documented `rent-flow.spec.ts` consumer had dropped it). |
| Stripe Checkout redirect | `wallet-stripe-redirect.spec.ts` (`page.route('https://checkout.stripe.com/**')`) | External (checkout.stripe.com) | ✅ external-boundary — fulfills the document navigation triggered by `window.location.href = checkoutUrl` so the redirect assertion completes inside the harness; paired with a `page.route` on `POST /wallet/topup` that injects a fake `checkoutUrl` (the API can't produce one locally without STRIPE_SECRET_KEY; only the topup response is intercepted — GET /wallet stays real). |
