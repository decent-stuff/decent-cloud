...
/**
 * E2E coverage for the contact delete flow (inline two-step confirm).
 *
 * ContactsEditor previously used a native confirm() dialog on row delete —
 ...
 *
 * Serial mode: the spec seeds + deletes account_contacts rows for the shared
...
/** Seed one phone contact; returns its numeric id. */
async function seedContact(accountHex: string, value: string): Promise<string> {
	const out = await sql(`
		INSERT INTO account_contacts (account_id, contact_type, contact_value)
		VALUES (decode('${accountHex}', 'hex'), 'phone', '${value.replace(/'/g, "''")}')
		RETURNING id
	`);
	const id = out.split('\n').map((l) => l.trim()).find((l) => /^\d+$/.test(l));
	if (!id) throw new Error(`seedContact did not RETURN an id; got: ${out}`);
	return id;
}

/** Remove any contacts left for the account (cleanup). */
async function cleanupContacts(accountHex: string): Promise<void> {
	await sql(`DELETE FROM account_contacts WHERE account_id = decode('${accountHex}', 'hex')`);
}

test.describe('Contact delete (inline two-step confirm)', () => {
	test('first Delete click reveals an inline confirm; second click deletes the contact', async ({ page, testAccount }) => {
		const accountHex = await accountIdHex(testAccount.username);
		const value = `+1-E2E-DEL-${Date.now()}`;
		const id = await seedContact(accountHex, value);
		try {
			...
			const row = page.locator('div.bg-surface-elevated.flex', { hasText: value });
			...
			await cleanupContacts(accountHex);
		}
	});

	test('Cancel aborts the deletion and keeps the contact', async ({ page, testAccount }) => {
		const accountHex = await accountIdHex(testAccount.username);
		const value = `+1-E2E-KEEP-${Date.now()}`;
		const id = await seedContact(accountHex, value);
		try {
			page.on('dialog', (d) => expect(d.type(), 'native dialog must not fire').toBe('never'));
