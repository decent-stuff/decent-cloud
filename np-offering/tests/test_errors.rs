...
fn test_error_creation() {
    ...
    assert!(matches!(offering_json_error, OfferingError::SerdeJsonError(_)));
    
    // Test InvalidPubkeyLength creation
    let pubkey_error = OfferingError::InvalidPubkeyLength(16);
    assert!(matches!(pubkey_error, OfferingError::InvalidPubkeyLength(_)));
    
    // Test OfferingNotFound creation
    let offering_not_found = OfferingError::OfferingNotFound("provider1".to_string(), "key1".to_string());
    assert!(matches!(offering_not_found, OfferingError::OfferingNotFound(_, _)));
    
    // Test ProviderNotFound creation
    let provider_not_found = OfferingError::ProviderNotFound("provider1".to_string());
    assert!(matches!(provider_not_found, OfferingError::ProviderNotFound(_)));
}
...
fn test_error_display() {
    ...
    let pubkey_error = OfferingError::InvalidPubkeyLength(16);
    assert_eq!(format!("{}", pubkey_error), "Invalid provider pubkey: expected 32 bytes, got 16");
    
    // Test OfferingNotFound display
    let offering_not_found = OfferingError::OfferingNotFound("provider1".to_string(), "key1".to_string());
    assert_eq!(format!("{}", offering_not_found), "Offering not found: provider=provider1, key=key1");
    
    // Test ProviderNotFound display
    let provider_not_found = OfferingError::ProviderNotFound("provider1".to_string());
    assert_eq!(format!("{}", provider_not_found), "Provider not found: provider1");
}
