...
fn test_registry_basic_operations() {
    ...
    assert_eq!(
        offering.unwrap().server_offering.offer_name,
        "Intel Dual Core Dedicated Server"
    );
...
fn test_registry_search() {
    ...
    let query = SearchQuery::new().with_filter(OfferingFilter::ProductType(ProductType::VPS));
    let results = registry.search(&query);
    assert_eq!(results.len(), 3);

    let query = SearchQuery::new().with_filter(OfferingFilter::HasGPU(true));
    let results = registry.search(&query);
    assert_eq!(results.len(), 1);

    let query = SearchQuery::new().with_filter(OfferingFilter::PriceRange(100.0, 150.0));
    let results = registry.search(&query);
    assert_eq!(results.len(), 1);
}
...
New Offering,New description.,NEW001,https://test.com/NEW001/,EUR,75.00,0.0,Visible,Cloud,None,Monthly,In stock,AMD,1,1,3.0 GHz,AMD Ryzen,non-ECC,DDR4,4 GB,0,0,1,100 GB,Standard,100 mbit,1024,DE,"Berlin, Germany","52.5200,13.4050","Basic support","Ubuntu, Debian",,,"PayPal""#;
    ...
    let count = registry
        .update_provider_offerings(provider.clone(), new_csv)
        .unwrap();
    assert_eq!(count, 1);
...
fn test_multiple_providers() {
    ...
    registry
        .add_provider_offerings(provider1.clone(), SAMPLE_CSV)
        .unwrap();
    registry
        .add_provider_offerings(provider2.clone(), SAMPLE_CSV)
        .unwrap();
