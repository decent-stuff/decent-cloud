use np_offering::{OfferingRegistry, ProviderPubkey, SearchQuery, OfferingFilter, ProductType, Currency, StockStatus, VirtualizationType};
mod common;
use common::*;
...
fn test_input_sanitization() {
    ...
    assert_eq!(results.len(), 2, "Expected 2 results for SQL injection input, got {}", results.len()); // Should find "Intel" offerings
    ...
    assert_eq!(results.len(), 2, "Expected 2 results for special chars, got {}", results.len()); // Should find "Intel" offerings
    ...
    assert_eq!(results.len(), 1, "Expected 1 result for netherlands, got {}", results.len()); // Should find the Netherlands offering
}
...
fn test_comprehensive_filter_combinations() {
    ...
    let results = registry.search(&query);
    assert_eq!(results.len(), 1);
    assert!(std::mem::discriminant(&results[0].server_offering.currency) == std::mem::discriminant(&Currency::USD));
    assert!(std::mem::discriminant(&results[0].server_offering.product_type) == std::mem::discriminant(&ProductType::VPS));
    
    // Test with virtualization type filter
    let query = SearchQuery::new()
        .with_filter(OfferingFilter::VirtualizationType(VirtualizationType::KVM));
...
fn test_case_insensitive_search() {
    ...
    for (search_term, expected_count) in test_cases {
        ...
        println!("Search for '{}': {} results (expected {})", search_term, results.len(), expected_count);
        assert_eq!(results.len(), expected_count, "Failed for search term: {}", search_term);
    }
}
...
fn test_stop_word_filtering() {
    ...
    let stop_words = vec!["the", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by"];
    
    for stop_word in stop_words {
        let results = registry.search_text(stop_word);
        assert_eq!(results.len(), 0, "Stop word '{}' should return no results", stop_word);
    }
    ...
    for short_word in short_words {
        ...
        assert_eq!(results.len(), 0, "Short word '{}' should return no results", short_word);
    }
}
