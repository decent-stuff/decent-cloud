...
-- replayed ICPay webhook (or any caller) could write an arbitrary string and
-- confuse every downstream guard that compares `== 'succeeded'` (provisioning
-- gate, refund gating, dispute handling). The code-level validation in
-- `update_icpay_payment_status` is the loud, caller-visible check; this CHECK
-- constraint is the un-bypassable backstop that holds even via direct SQL.
--
