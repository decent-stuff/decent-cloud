...
async fn doctor_command() -> Result<(), std::io::Error> {
    ...
    // === Catalog Integrity ===
    // (The synthetic example-offering guard that lived here was removed: migration
    // `053_drop_example_provider_seed.sql` now drops demo rows at source, so a
    // runtime doctor check is no longer needed. See docs/PRODUCT-DIRECTION.md F2.)
