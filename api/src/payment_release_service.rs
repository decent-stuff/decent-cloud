...
impl PaymentReleaseService {
    ...
    async fn process_releases_once(&self) -> anyhow::Result<()> {
        ...
        tracing::info!("Found {} contracts ready for payment release", contracts.len());
        ...
        for contract in contracts {
            ...
            let last_release = contract.last_release_at_ns.unwrap_or(
                contract.start_timestamp_ns.unwrap_or(contract.created_at_ns)
            );
            let period_start_ns = last_release;
            let period_end_ns = current_timestamp_ns;

            // Calculate total contract duration
            let start = contract.start_timestamp_ns.unwrap_or(contract.created_at_ns);
            let end = contract.end_timestamp_ns.unwrap_or(current_timestamp_ns);
            let total_duration_ns = end - start;

            if total_duration_ns <= 0 {
                tracing::warn!("Contract {} has invalid duration, skipping", contract.contract_id);
                continue;
            }
            ...
            if release_amount_e9s <= 0 {
                tracing::debug!("Contract {} has no earnings to release", contract.contract_id);
                continue;
            }

            // Create payment release record
            match self.database.create_payment_release(
                &contract_id_bytes,
                "daily",
                period_start_ns,
                period_end_ns,
                ...
                Ok(release) => {
                    ...
                    let new_total_released = contract.total_released_e9s.unwrap_or(0) + release_amount_e9s;
                    if let Err(e) = self.database.update_contract_release_tracking(
                        &contract_id_bytes,
