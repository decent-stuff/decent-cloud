...
fn test_release_calculation_half_time_elapsed() {
    ...
    let payment_amount_e9s: i64 = 100;
    let start_ns: i64 = 0;
    let end_ns: i64 = 30 * 24 * 3600 * 1_000_000_000; // 30 days in ns
    let current_ns: i64 = 15 * 24 * 3600 * 1_000_000_000; // 15 days in ns
    let last_release_ns = start_ns;

    let period_duration_ns = current_ns - last_release_ns;
    let total_duration_ns = end_ns - start_ns;
    // Use integer arithmetic to match production implementation
    let release_amount = ((payment_amount_e9s as i128) * (period_duration_ns as i128)
...
fn test_release_calculation_one_day_out_of_thirty() {
    ...
    let payment_amount_e9s: i64 = 720;
    let start_ns: i64 = 0;
    let end_ns: i64 = 30 * 24 * 3600 * 1_000_000_000;
    let current_ns: i64 = 24 * 3600 * 1_000_000_000;
    let last_release_ns = start_ns;
...
fn test_release_calculation_daily_incremental() {
    ...
    let payment_amount_e9s: i64 = 300;
    let start_ns: i64 = 0;
    let end_ns: i64 = 10 * 24 * 3600 * 1_000_000_000;
    let total_duration_ns = end_ns - start_ns;
    ...
    for day in 1..=10 {
        let current_ns: i64 = day * 24 * 3600 * 1_000_000_000;
        let period_duration_ns = current_ns - last_release_ns;
        let release_amount = ((payment_amount_e9s as i128) * (period_duration_ns as i128)
            / (total_duration_ns as i128)) as i64;
...
fn test_release_calculation_no_time_elapsed() {
    ...
    let payment_amount_e9s: i64 = 1000;
    let start_ns: i64 = 0;
    let end_ns: i64 = 30 * 24 * 3600 * 1_000_000_000;
