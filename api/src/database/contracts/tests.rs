...
#[tokio::test]
async fn test_update_stripe_payment_intent() {
    let db = setup_test_db().await;
    let contract_id = vec![100u8; 32];
    let requester_pk = vec![1u8; 32];
    let provider_pk = vec![2u8; 32];

    // Insert a contract without Stripe payment intent
    insert_contract_request(
        &db,
        &contract_id,
        &requester_pk,
    ...
    // Verify contract has no payment intent initially
    let contract = db.get_contract(&contract_id).await.unwrap().unwrap();
    assert_eq!(contract.stripe_payment_intent_id, None);

    // Update with Stripe payment intent ID
    let payment_intent_id = "pi_test_123456789";
...
#[tokio::test]
async fn test_update_stripe_payment_intent_overwrites() {
    let db = setup_test_db().await;
    ...
    let first_intent = "pi_first_123";
    db.update_stripe_payment_intent(&contract_id, first_intent)
        .await
        .unwrap();

    // Overwrite with new payment intent
    let second_intent = "pi_second_456";
    db.update_stripe_payment_intent(&contract_id, second_intent)
        .await
        .unwrap();
