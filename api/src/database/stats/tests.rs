...
async fn test_get_platform_stats_with_data() {
    ...
    sqlx::query("INSERT INTO provider_profiles (pubkey, name, api_version, profile_version, updated_at_ns) VALUES (?, 'Test', '1.0', '1.0', 0)")
    ...
    sqlx::query("INSERT INTO provider_check_ins (pubkey, memo, nonce_signature, block_timestamp_ns) VALUES (?, 'test', ?, ?)")
...
async fn test_get_reputation_with_changes() {
    ...
    sqlx::query("INSERT INTO reputation_changes (pubkey, change_amount, reason, block_timestamp_ns) VALUES (?, 10, 'test', 0)")
            .bind(&pubkey).execute(&db.pool).await.unwrap();
    sqlx::query("INSERT INTO reputation_changes (pubkey, change_amount, reason, block_timestamp_ns) VALUES (?, -5, 'penalty', 1000)")
            .bind(&pubkey).execute(&db.pool).await.unwrap();
...
async fn test_get_top_providers_by_reputation() {
    ...
    sqlx::query("INSERT INTO reputation_changes (pubkey, change_amount, reason, block_timestamp_ns) VALUES (?, 100, 'good', 0)")
            .bind(&pk1).execute(&db.pool).await.unwrap();
    sqlx::query("INSERT INTO reputation_changes (pubkey, change_amount, reason, block_timestamp_ns) VALUES (?, 50, 'ok', 0)")
...
async fn test_get_provider_stats_with_data() {
    ...
    sqlx::query("INSERT INTO contract_sign_requests (contract_id, requester_pubkey, requester_ssh_pubkey, requester_contact, provider_pubkey, offering_id, payment_amount_e9s, request_memo, created_at_ns, status) VALUES (?, ?, 'ssh-key', 'contact', ?, 'off-1', 1000, 'memo', 0, 'pending')")
            .bind(vec![1u8; 32]).bind(vec![2u8; 32]).bind(&pubkey).execute(&db.pool).await.unwrap();
    sqlx::query("INSERT INTO contract_sign_requests (contract_id, requester_pubkey, requester_ssh_pubkey, requester_contact, provider_pubkey, offering_id, payment_amount_e9s, request_memo, created_at_ns, status) VALUES (?, ?, 'ssh-key', 'contact', ?, 'off-1', 2000, 'memo', 1000, 'active')")
...
async fn test_get_latest_block_timestamp_ns_with_data() {
    ...
    sqlx::query("INSERT INTO provider_check_ins (pubkey, memo, nonce_signature, block_timestamp_ns) VALUES (?, 'test1', ?, ?)")
            .bind(&pubkey).bind(vec![0u8; 64]).bind(timestamp1).execute(&db.pool).await.unwrap();
    sqlx::query("INSERT INTO provider_check_ins (pubkey, memo, nonce_signature, block_timestamp_ns) VALUES (?, 'test2', ?, ?)")
