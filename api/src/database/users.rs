...
impl Database {
    ...
    pub async fn get_user_socials(&self, pubkey: &[u8]) -> Result<Vec<UserSocial>> {
        let socials = sqlx::query_as!(
            UserSocial,
            r#"SELECT id as "id!", platform, username, profile_url FROM user_socials WHERE user_pubkey = ?"#,
            pubkey
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn get_user_public_keys(&self, pubkey: &[u8]) -> Result<Vec<UserPublicKey>> {
        let keys = sqlx::query_as!(
            UserPublicKey,
            r#"SELECT id as "id!", key_type, key_data, key_fingerprint, label FROM user_public_keys WHERE user_pubkey = ?"#,
            pubkey
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn upsert_user_profile(
        ...
        sqlx::query!(
            r#"INSERT INTO user_profiles (pubkey, display_name, bio, avatar_url, updated_at_ns)
             VALUES (?, ?, ?, ?, ?)
    ...
    pub async fn delete_user_contact(&self, pubkey: &[u8], contact_id: i64) -> Result<()> {
        sqlx::query!(
            "DELETE FROM user_contacts WHERE user_pubkey = ? AND id = ?",
            pubkey,
            contact_id
        )
        .execute(&self.pool)
    ...
    pub async fn delete_user_social(&self, pubkey: &[u8], social_id: i64) -> Result<()> {
        sqlx::query!(
            "DELETE FROM user_socials WHERE user_pubkey = ? AND id = ?",
            pubkey,
            social_id
        )
        .execute(&self.pool)
    ...
    pub async fn delete_user_public_key(&self, pubkey: &[u8], key_id: i64) -> Result<()> {
        sqlx::query!(
            "DELETE FROM user_public_keys WHERE user_pubkey = ? AND id = ?",
            pubkey,
            key_id
        )
        .execute(&self.pool)
