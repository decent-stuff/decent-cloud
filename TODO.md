...
### Cloud Provisioning — Completed (cont.)

6. ~~**Store provisioning error in DB**~~ — Done: Migration 017 added `error_message TEXT` column to `cloud_resources`. `mark_cloud_resource_failed()` stores the error message. UI displays it in a red box when status is `failed`.
7. ~~**UI: show SSH access instructions**~~ — Done: Resources page displays `ssh -p {port} {user}@{ip}` command with gateway subdomain when available.
8. ~~**Auto-refresh for transient resources**~~ — Done: Resources page polls every 10s when resources are in `provisioning` or `deleting` state. Polling stops automatically when all resources reach terminal states.

### Cloud Provisioning — Known Limitations

9. **Multi-instance race** — If two API server instances share the same DB (e.g., dev + local during testing), both provisioning services race on the same resources. The 10-minute lock timeout prevents corruption but can cause delayed provisioning or double-attempt waste. Not a prod issue if only one instance runs, but fragile.
10. **Hetzner server type availability is location-dependent** — CPX (old gen) types don't work in all locations. CX23+ (new gen) work in nbg1, hel1, fsn1. No server-side validation before sending to Hetzner API; user sees a cryptic "unsupported location for server type" error.

### Cloud Provisioning — Longer-term

11. **Recipe marketplace UI** — Website needs UI for browsing/purchasing recipe offerings and viewing provisioned recipe instances with connection details.
12. **Recipe script versioning** — Scripts are snapshotted at contract creation. Consider a `recipe_versions` table so authors can update scripts and buyers can upgrade.
