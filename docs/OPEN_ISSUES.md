...
| **A10** | **#477 — marketplace-empty-state e2e parallel-contamination flake** | Two layers: (1) cross-RUN leak — FIXED in `062be50d` (search-dsl beforeAll now reclaims stale `e2edsl-*` rows + afterAll no longer silently swallows cleanup errors). (2) within-RUN parallel contamination — OPEN: `search-dsl.spec.ts` seeds 6 always-online (`self_provisioned`) offerings while `marketplace-empty-state.spec.ts` (separate worker, same DB) asserts the UNFILTERED marketplace shows zero online offerings + a reveal button. The reveal button only renders with NO active filter, so `?q=` scoping is ruled out. Fix direction: Playwright project/worker split so no sibling spec seeds visible offerings while this spec runs. Pre-existing, tangential; full root-cause + repro in #477. |
| ~~**A11**~~ | **~~Pre-pay wallet (stored-value billing)~~** **DONE (2026-08-10)** | Full MVP: migrations 054 (wallet_balances + wallet_ledger) + 055 (refund-gate wallet exemption); `PaymentMethod::Wallet` variant; top-up via Stripe Checkout → webhook credit; rentals debit wallet at creation (replaces per-contract checkout); instant refund-to-balance on cancel/reject/provisioning-failed; `/dashboard/wallet` UI + dashboard card + sidebar. Plan: `docs/plans/2026-08-10-prepay-wallet.md`. 325 e2e + 101 nextest pass. **Follow-ups (not blocking):** (a) webhook top-up idempotency (key credit on session id — potential double-credit on Stripe replay); (b) auto-renewal wallet-debit path; (c) verify spending-alert cap interaction with instant-debit model. |
