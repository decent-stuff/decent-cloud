...
| **TD-1** | #444 — Large source-file splits (>2000 lines) | **5** | **L** | `dc-agent/src/main.rs` **3681→139 DONE**. `database/providers.rs` **already split** into `database/providers/` dir. `database/offerings.rs` (2986L) has a clean split plan (`docs/plans/2026-08-13-providers-offerings-split-design.md`, 5 M-effort subtasks). `openapi/providers.rs` (4325L) needs its own higher-risk plan (Path-A `#[OpenApi]`, spec-snapshot concerns). CLI snapshot guard locks `--help` byte-identity. |
| **UX-EMAIL** | Operator email still `reseller-npfovd@dev.test` (stale, fake, user-facing) | **7** | **S** | Appears in Account→Overview + Support page. **BLOCKED on operator** — need a real monitored email address to replace the dev.test placeholder. |
| **UX-EMPTY** | Marketplace empty-state unhelpful + single-provider fragility | **5** | **M** | "0 offerings found" has no guidance; with one real provider, a brief heartbeat gap flips the entire marketplace empty. Fix: contextual empty-state message + show offline offerings greyed-out rather than hiding. |
| **UX-SUPPORT** | Support page stacked error states look broken when Chatwoot unavailable | **5** | **M** | Shows "not configured" + "Unable to load portal status" + empty wizard simultaneously. Fix: single friendly non-error state when Chatwoot is down. |
| **UX-SMALL-SAMPLE** | Analytics/red-flags show misleading precision from tiny samples | **4** | **S** | "100.00% conversion" from 2 views; "100% cancellation rate" from 2 test rentals. Fix: show "—" or "insufficient data" below a minimum-sample threshold (e.g. <10). |
| **UX-FOOTER** | Footer "Offerings" link dead-ends at login wall for anonymous users | **4** | **S** | Public footer link → login-gated page (shows "Login Required"). Fix: hide/relabel for anonymous, or point at the public marketplace. |
